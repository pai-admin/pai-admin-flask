...
def init_route(bp: Blueprint) -> None:
    ...
    # 修改密码
    bp.add_url_rule("/editPwd", methods=["POST"], endpoint="edit_pwd", view_func=account.edit_pwd)
    # 菜单权限
    bp.add_url_rule("/auth", methods=["GET"], endpoint="auth", view_func=account.auth)
    # 修改资料
    bp.add_url_rule("/info/edit", methods=["POST"], endpoint="edit_info", view_func=account.edit_info)
    # 个人日志
    bp.add_url_rule("/log/my", methods=["GET"], endpoint="my_log", view_func=account.my_log)
    # 日志列表
